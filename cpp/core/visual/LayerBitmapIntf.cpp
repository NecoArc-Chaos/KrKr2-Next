...
bool iTVPBaseBitmap::Copy9Patch(const iTVPBaseBitmap *ref, tTVPRect &margin) {
    ...
    if(src_center_width == 1) { // コピー元の幅が1の時はその色で塗りつぶす
        for(tjs_int y = 0; y < src_top_height; y++) {
...
bool iTVPBaseBitmap::AffineBlt(tTVPRect destrect, const iTVPBaseBitmap *ref,
    ...
    auto type = (tTVPBBStretchType)(mode & stTypeMask);
    // Always use this bitmap's own render manager. Do NOT fall back to
    // software when source uses a different render manager, because the
    // target textures (created by this->GetRenderManager()) would be
    // incompatible with the fallback manager's OperateTriangles, which
    // performs unsafe static_casts on textures.
    //
...
void tTVPBaseBitmap::DoBoxBlurLoop(const tTVPRect &rect, const tTVPRect & area)
{
	...
	tARGB * vert_sum = nullptr; // vertical sum of the pixel
	tjs_uint32 * * dest_buf = nullptr; // destination pixel temporary buffer
