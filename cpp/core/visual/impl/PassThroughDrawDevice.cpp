...
/*
   ...
   Window.PassThroughDrawDevice.dtDBD3D Direct3DによるDB
        ...
        drawer
                現在使用されている drawer
   を表すプロパティ。以下のいずれかの値をとる。 読み取り専用。
                Window.PassThroughDrawDevice.dtNone
   普通はこれはない Window.PassThroughDrawDevice.dtDrawDib
   ダブルバッファリング(DB)なし Window.PassThroughDrawDevice.dtDBGDI
   GDIによるDB Window.PassThroughDrawDevice.dtDBDD
...
//---------------------------------------------------------------------------
tTVPPassThroughDrawDevice::~tTVPPassThroughDrawDevice() { DestroyDrawer(); }
//---------------------------------------------------------------------------
...
void tTVPPassThroughDrawDevice::EnsureDrawer() {
    ...
    // if(TargetWindow)
    {
        ...
        if(!Drawer)
            need_recreate = true;
        if(zoom_was_required != zoom_is_required)
            need_recreate = true;
        //		if(need_recreate) should_benchmark = true;
        //		if(SrcSizeChanged) { SrcSizeChanged = false;
        // need_recreate = true;
        //}
