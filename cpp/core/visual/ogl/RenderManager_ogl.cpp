...
#define GLAPIENTRY
    ...
    typedef void(GLAPIENTRY fClearTexImage)(uint texture, int level,
                                            GLenum format, GLenum type,
                                            const void *data);
    static fClearTexImage *glClearTexImage;
    typedef void(GLAPIENTRY fClearTexSubImage)(uint texture, int level,
                                               int xoffset, int yoffset,
                                               int zoffset, GLsizei width,
                                               GLsizei height, GLsizei depth,
                                               GLenum format, GLenum type,
                                               const void *data);
...
static void TVPInitGLExtensionFunc() {
    ...
    if(GL::glGetProcAddress) {
        ...
        if(!GL::glCopyImageSubData &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_EXT_copy_image))
            GL::glCopyImageSubData =
                (GL::fCopyImageSubData *)GL::glGetProcAddress(
                    "glCopyImageSubDataEXT");
        if(!GL::glCopyImageSubData &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_OES_copy_image))
            GL::glCopyImageSubData =
                (GL::fCopyImageSubData *)GL::glGetProcAddress(
                    "glCopyImageSubDataOES");
        if(!GL::glCopyImageSubData &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_NV_copy_image))
            GL::glCopyImageSubData =
                (GL::fCopyImageSubData *)GL::glGetProcAddress(
                    "glCopyImageSubDataNV");
        if(!GL::glCopyImageSubData &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_ARB_copy_image))
            GL::glCopyImageSubData =
                (GL::fCopyImageSubData *)GL::glGetProcAddress(
                    "glCopyImageSubData");

        if(!GL::glClearTexImage &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_EXT_clear_texture)) {
            GL::glClearTexImage = (GL::fClearTexImage *)GL::glGetProcAddress(
                "glClearTexImageEXT");
        ...
        if(!GL::glClearTexImage &&
           TVPCheckGLExtension(UsedGLExtInfo.GLEXT_ARB_clear_texture)) {
            GL::glClearTexImage =
                (GL::fClearTexImage *)GL::glGetProcAddress("glClearTexImage");
            GL::glClearTexSubImage =
                (GL::fClearTexSubImage *)GL::glGetProcAddress(
                    "glClearTexSubImage");
        }
...
#endif
    if(!GL::glAlphaFunc &&
       TVPCheckGLExtension(UsedGLExtInfo.GLEXT_QCOM_alpha_test))
        GL::glAlphaFunc =
            (GL::fAlphaFunc *)GL::glGetProcAddress("glAlphaFuncQCOM");
}
...
std::string TVPGetOpenGLInfo() {
    ...
    ret << int(maxTextSize);
    ret << "\n";
    ret << LocaleConfigManager::GetInstance()->GetText(
        "supported_opengl_extension");
    for(const char *const *name = (&UsedGLExtInfo.NameBegin) + 1; *name;
        ++name) {
...
// "GL_EXT_shader_framebuffer_fetch")) { 		ret = true; if
// (forceNotice ||
// GlobalConfigManager::GetInstance()->GetValue<int>("noticed_adreno_issue",
// 0) < 1) { 			const char *btnText[2] = {
...
// 				("issue_GL_EXT_shader_framebuffer_fetch").c_str(),
// "Info", forceNotice ? 1 : 2, btnText); 			if (n == 1) {
// 				GlobalConfigManager::GetInstance()->SetValueInt("noticed_adreno_issue",
// 1); GlobalConfigManager::GetInstance()->SaveToFile();
// 			}
// 		}
// 	}
...
static tjs_uint8 *TVPShrinkXBy2(tjs_uint *dpitch, const tjs_uint8 *src,
                                tjs_int spitch, tjs_uint srcw, tjs_uint srch) {
...
static tjs_uint8 *TVPShrinkYBy2(tjs_uint *dpitch, const tjs_uint8 *src,
                                tjs_int spitch, tjs_uint srcw, tjs_uint srch) {
...
static tjs_uint8 *TVPShrinkXYBy2_8(tjs_uint *dpitch, const tjs_uint8 *src,
                                   tjs_int spitch, tjs_uint srcw,
                                   tjs_uint srch) {
    tjs_uint dstw = (srcw + 1) / 2, dsth = (srch + 1) / 2;
...
static tjs_uint8 *TVPShrinkXBy2_8(tjs_uint *dpitch, const tjs_uint8 *src,
                                  tjs_int spitch, tjs_uint srcw,
                                  tjs_uint srch) {
    tjs_uint dstw = (srcw + 1) / 2;
    tjs_uint dp = *dpitch = (dstw + 7) & ~7;
...
static tjs_uint8 *TVPShrinkYBy2_8(tjs_uint *dpitch, const tjs_uint8 *src,
                                  tjs_int spitch, tjs_uint srcw,
                                  tjs_uint srch) {
    tjs_uint dsth = (srch + 1) / 2;
...
static tjs_uint8 *TVPShrink(tjs_uint *dpitch, const tjs_uint8 *src,
                            tjs_int spitch, tjs_uint srcw, tjs_uint srch,
...
static tjs_uint8 *TVPShrink_8(tjs_uint *dpitch, const tjs_uint8 *src,
                              tjs_int spitch, tjs_uint srcw, tjs_uint srch,
                              tjs_uint dstw, tjs_uint dsth) {
    if(srcw == dstw) {
        if((srch + 1) / 2 == dsth) {
...
static bool TVPCheckSolidPixel(const tjs_uint8 *pixel, tjs_int pitch, tjs_int w,
                               tjs_int h) {
    const tjs_uint8 *p = pixel;
...
public:
    tTVPOGLTexture2D_split(tTVPBitmap *bmp) :
        tTVPOGLTexture2D(
            ...
            0 /*don't generate texture*/) { // only accept from bitmap
        Bitmap = bmp;
        Bitmap->AddRef();
    }
    ...
    tjs_uint32 GetPoint(int x, int y) override {
        if(Bitmap->Is32bit()) {
            ...
            TVPThrowExceptionMessage(
                TJS_W("Unsupported texture format[RGB] for getting "
                      "point color."));
            return 0;
        }
    }
    ...
    void UpdateTextureData(GLTextureInfo &texinfo, const tTVPRect &rc) {
        ...
        if(texinfo.Width > GetMaxTextureWidth()) {
            ...
            if(GL_CHECK_unpack_subimage) {
                ...
                pix =
                    new unsigned char[texinfo.Width * texinfo.Height * pixsize];
                unsigned char *src =
                    (unsigned char *)Bitmap->GetScanLine(rc.top);
...
public:
    // for manual init
    tTVPOGLTexture2D_static(TVPTextureFormat::e format, unsigned int tw,
                            unsigned int th, float sw, float sh,
                            GLint mode = GL_LINEAR) :
    ...
    tTVPOGLTexture2D_static(const void *pixel, int pitch, unsigned int iw,
                            unsigned int ih, TVPTextureFormat::e format,
                            unsigned int tw, unsigned int th, float sw,
                            float sh, GLint mode = GL_LINEAR) :
        tTVPOGLTexture2D(tw, th, format, mode) {
        _scaleW = sw;
        _scaleH = sh;
    ...
    tTVPOGLTexture2D_static(const void *data, int len, GLenum format,
                            unsigned int tw, unsigned int th, unsigned int iw,
                            unsigned int ih, float sw, float sh) :
    ...
    void InitCompressedPixel(const void *data, unsigned int len, GLenum format,
    ...
    void InitPixel(const void *pixel, unsigned int pitch, GLenum format,
    ...
    void SetSize(unsigned int w, unsigned int h) override { assert(false); }
...
public:
    tTVPOGLTexture2D_mutatble(const void *pixel, int pitch, unsigned int w,
        ...
        tTVPOGLTexture2D(
            w, h,
            format == TVPTextureFormat::RGB ? TVPTextureFormat::RGBA : format,
            GL_LINEAR) {
        if(!pixel) {
            _scaleW = sw;
            _scaleH = sh;
            unsigned int intw = power_of_two(w * sw),
                         inth = power_of_two(h * sh);
            if(intw < w)
...
protected:
    ...
    // 	typedef
    // bool(tTVPOGLRenderMethod::*FCustomProc)(tTVPOGLTexture2D *,
    // const tTVPRect&, const std::vector<GLVertexInfo>&); FCustomProc
    // CustomProc;
    bool tar_as_src;
...
public:
    ...
    tTVPOGLRenderMethod *SetBlendFuncSeparate(int func, int srcRGB, int dstRGB,
                                              int srcAlpha,
                                              int dstAlpha) override {
        BlendFunc = func;
        BlendSrcRGB = srcRGB;
        BlendDstRGB = dstRGB;
        BlendSrcA = srcAlpha;
    ...
    bool IsBlendTarget() override { return !!BlendFunc; }
    ...
    bool (*CustomProc)(tTVPOGLRenderMethod *_method, tTVPOGLTexture2D *_tar,
                       tTVPOGLTexture2D *reftar, const tTVPRect *rctar,
...
public:
    ...
    static GLenum GetVertShader(unsigned int nTex) {
        ...
        if(!m_cachedSahder[nTex]) {
            ...
            shader << "}";
            m_cachedSahder[nTex] =
                CompileShader(GL_VERTEX_SHADER, shader.str());
        }
...
public:
    ...
    int EnumParameterID(const char *name) override { return 0; }
    void SetParameterColor4B(int id, unsigned int clr) override {
        if(CustomProc)
            m_color = clr;
        else
    ...
    static bool DoFill(tTVPOGLRenderMethod *method, tTVPOGLTexture2D *tar,
                       tTVPOGLTexture2D *, const tTVPRect *rctar,
                       const tRenderTexRectArray &) {
        uint32_t c =
            static_cast<tTVPOGLRenderMethod_FillARGB *>(method)->m_color;
        uint8_t clr[4] = { (uint8_t)c, (uint8_t)(c >> 8), (uint8_t)(c >> 16),
                           (uint8_t)(c >> 24) };
        if(rctar->left <= 0 && rctar->top <= 0 &&
           rctar->get_width() >= tar->GetWidth() &&
           rctar->get_height() >= tar->GetHeight()) {
            GL::glClearTexImage(tar->texture, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                clr);
        } else {
            float sw, sh;
            tar->GetScale(sw, sh);
            if(sw != 1.f && sh != 1.f)
                return false;
            GL::glClearTexSubImage(tar->texture, 0, rctar->left, rctar->top, 0,
                                   rctar->get_width(), rctar->get_height(), 1,
                                   GL_RGBA, GL_UNSIGNED_BYTE, clr);
        }
        CHECK_GL_ERROR_DEBUG();
        // #ifdef _DEBUG
        //         static bool check = false;
        //         if(check) {
        //             cv::Mat _src(tar->GetInternalHeight(),
        //             tar->GetInternalWidth(),
        //                          CV_8UC4);
        //             GL::glGetTextureImage(tar->texture, 0, GL_BGRA,
        //             GL_UNSIGNED_BYTE,
        //                                   tar->GetInternalHeight() *
        //                                       tar->GetInternalWidth() * 4,
        //                                   _src.ptr(0, 0));
        //         }
        // #endif
        return true;
...
public:
    ...
    void onFinish() override { glDisable(GL_ALPHA_TEST); }
};
...
class tTVPOGLRenderMethod_BoxBlur : public tTVPOGLRenderMethod_Script {
    ...
    void ApplyTexture(unsigned int i, const GLVertexInfo &info) override {
        if(i == 0) {
            ...
            float pos[8 * 2];
            float l = area.left / w, t = area.top / h, r = area.right / w,
                  b = area.bottom / h;
            pos[0] = l;
            pos[1] = t;
            pos[2] = 0;
...
bool tTVPOGLTexture2D::RestoreNormalSize() {
    ...
    if(w < GetMaxTextureWidth() && h < GetMaxTextureHeight()) {
        ...
        static tTVPOGLRenderMethod *method =
            (tTVPOGLRenderMethod *)TVPGetRenderManager()->GetRenderMethod(
                "Copy");
        method->Apply();
...
static iTVPTexture2D *(*_CreateStaticTexture2D)(const void *dib, tjs_uint tw,
                                                ...
                                                bool isOpaque);
static iTVPTexture2D *(*_CreateMutableTexture2D)(const void *pixel, int pitch,
...
protected:
    ...
    template <typename T>
    T *CompileAndRegScript(const char *name, const std::string &script,
                           int nTex, const char *prefix = nullptr) {
        T *method = new T();
    ...
    T *CompileAndRegRegularBlendMethod(const char *name,
        ...
        if(GL_CHECK_shader_framebuffer_fetch) {
            return CompileAndRegScript<T>(
                name, prefix + ScriptCommonFBFBltPrefix + code, 1,
                common_shader_framebuffer_fetch_prefix);
        } else {
            T *method = CompileAndRegScript<T>(
                name, prefix + ScriptCommonBltPrefix + code, 2);
            method->SetTargetAsSrc();
            return method;
        }
    }
    ...
    static bool glew_dynamic_binding() {
        ...
        // If the current opengl driver doesn't have framebuffers
        // methods, check if an extension exists
        if(glGenFramebuffers == nullptr) {
            TVPConsoleLog("OpenGL: glGenFramebuffers is nullptr, try to detect "
                          "an extension");
            if(strstr(gl_extensions, "ARB_framebuffer_object")) {
                ...
                glIsRenderbuffer = (PFNGLISRENDERBUFFERPROC)wglGetProcAddress(
                    "glIsRenderbuffer");
                glBindRenderbuffer =
                    (PFNGLBINDRENDERBUFFERPROC)wglGetProcAddress(
                        "glBindRenderbuffer");
                glDeleteRenderbuffers =
                    (PFNGLDELETERENDERBUFFERSPROC)wglGetProcAddress(
                        "glDeleteRenderbuffers");
                glGenRenderbuffers =
                    (PFNGLGENRENDERBUFFERSPROC)wglGetProcAddress(
                        "glGenRenderbuffers");
                glRenderbufferStorage =
                    (PFNGLRENDERBUFFERSTORAGEPROC)wglGetProcAddress(
                        "glRenderbufferStorage");
                glGetRenderbufferParameteriv =
                    (PFNGLGETRENDERBUFFERPARAMETERIVPROC)wglGetProcAddress(
                        "glGetRenderbufferParameteriv");
                glIsFramebuffer = (PFNGLISFRAMEBUFFERPROC)wglGetProcAddress(
                    "glIsFramebuffer");
                glBindFramebuffer = (PFNGLBINDFRAMEBUFFERPROC)wglGetProcAddress(
                    "glBindFramebuffer");
                glDeleteFramebuffers =
                    (PFNGLDELETEFRAMEBUFFERSPROC)wglGetProcAddress(
                        "glDeleteFramebuffers");
                ...
                glFramebufferTexture1D =
                    (PFNGLFRAMEBUFFERTEXTURE1DPROC)wglGetProcAddress(
                        "glFramebufferTexture1D");
                glFramebufferTexture2D =
                    (PFNGLFRAMEBUFFERTEXTURE2DPROC)wglGetProcAddress(
                        "glFramebufferTexture2D");
                glFramebufferTexture3D =
                    (PFNGLFRAMEBUFFERTEXTURE3DPROC)wglGetProcAddress(
                        "glFramebufferTexture3D");
                ...
                glGetFramebufferAttachmentParameteriv =
                    (PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVPROC)
                        wglGetProcAddress("glGetFramebufferAttachment"
                                          "ParameterivEXT");
                glGenerateMipmap = (PFNGLGENERATEMIPMAPPROC)wglGetProcAddress(
                    "glGenerateMipmapEXT");
            } else {
    ...
    static iTVPTexture2D *CreateStaticTexture2D(const void *dib, tjs_uint tw,
                                                tjs_uint th, tjs_int pitch,
                                                TVPTextureFormat::e fmt,
                                                tjs_uint dstw, tjs_uint dsth,
                                                bool isOpaque) {
        ...
        tTVPOGLTexture2D_static *ret = new tTVPOGLTexture2D_static(
            pixel, pitch, w, h, fmt, tw, th, sw, sh);
        if(tmp)
            delete[] tmp;
        return ret;
    }
    ...
    static iTVPTexture2D *CreateStaticTexture2D_ETC2(const void *dib,
                                                     tjs_uint w, tjs_uint h,
                                                     tjs_int pitch,
                                                     TVPTextureFormat::e fmt,
                                                     bool isOpaque) {
        ...
        if(w > GetMaxTextureWidth() || h > GetMaxTextureHeight()) {
            ...
            n = (h + GetMaxTextureHeight() - 1) / GetMaxTextureHeight();
            int th = (h + n - 1) / n;
            return CreateStaticTexture2D(dib, w, h, pitch, fmt, tw, th,
                                         isOpaque);
        }
        if(w * h < 16 * 16) {
        ...
        if(isOpaque) {
            pixel =
                (uint8_t *)ETCPacker::convert(dib, w, h, pitch, true, pvrsize);
            texfmt = GL_COMPRESSED_RGB8_ETC2;
        } else {
            pixel = (uint8_t *)ETCPacker::convertWithAlpha(dib, w, h, pitch,
                                                           pvrsize);
            texfmt = GL_COMPRESSED_RGBA8_ETC2_EAC;
        }

        ret = new tTVPOGLTexture2D_static(pixel, pvrsize, texfmt, w, h, tw, th,
                                          1, 1);
        if(pixel)
            delete[] pixel;
        return ret;
    ...
    static iTVPTexture2D *CreateStaticTexture2D_PVRTC(const void *dib,
                                                      tjs_uint w, tjs_uint h,
                                                      tjs_int pitch,
                                                      TVPTextureFormat::e fmt,
                                                      bool isOpaque) {
        ...
        tjs_uint32 *inBuf =
            (tjs_uint32 *)TJSAlignedAlloc(pvrPixels * sizeof(tjs_uint32), 4);
        ...
        ret = new tTVPOGLTexture2D_static(
            outBuf, pvrSize,
            isOpaque ? GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG
    ...
    static iTVPTexture2D *CreateMutableTexture2D(const void *pixel, int pitch,
                                                 unsigned int w, unsigned int h,
                                                 float sw, float sh,
                                                 TVPTextureFormat::e fmt) {
    ...
    CreateMutableTexture2D_normal(const void *pixel, int pitch, unsigned int w,
                                  unsigned int h, TVPTextureFormat::e fmt) {
        float sw = 1.f, sh = 1.f;
    ...
    static iTVPTexture2D *CreateMutableTexture2D_half(const void *pixel,
                                                      int pitch, unsigned int w,
                                                      unsigned int h,
                                                      TVPTextureFormat::e fmt) {
        float sw = 1.f, sh = 1.f;
        if(!pixel) {
...
public:
    TVPRenderManager_OpenGL() : tempTexture(nullptr), _drawCount(0) {
        ...
        const std::string alpha_thresholdPrefix(
            "uniform float alpha_threshold;\n");
        const std::string colorPrefix("uniform vec4 color;\n");
        const std::string copyShader(
            "void main(){\n"
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + colorPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_SRC_ALPHA,
                                   GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE);
        TEST_SHADER(ApplyColorMap,
                    TVPApplyColorMap_HDA_o(testdest, testrule, 256 * 256,
                                           TEST_SHADER_COLOR, TEST_SHADER_OPA),
                    src_tex[0].first = texdata[2]);

        CompileAndRegRegularBlendMethod(
            "ApplyColorMap_d", opacityPrefix + colorPrefix,
            "    s.r *= opacity;\n"
            "    d.a = s.r + d.a - s.r * d.a;\n"
            "    d.rgb = mix(d.rgb, color.rgb, s.r / (d.a + "
            "0.0001));\n"
            "    gl_FragColor = d;\n"
            "}");
        TEST_SHADER(ApplyColorMap_d,
                    TVPApplyColorMap_do(testdest, testrule, 256 * 256,
                                        TEST_SHADER_COLOR, TEST_SHADER_OPA),
                    src_tex[0].first = texdata[2]);
        ...
        TEST_SHADER(ApplyColorMap_a,
                    TVPApplyColorMap_ao(testdest, testrule, 256 * 256,
                                        TEST_SHADER_COLOR, TEST_SHADER_OPA),
                    src_tex[0].first = texdata[2]);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + colorPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE, GL_ZERO,
                                   GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(
            RemoveOpacity,
        ...
        // ---------- Fill ----------
        std::string fillColorShader(colorPrefix +
                                    "void main(){\n"
                                    "    gl_FragColor = color;"
                                    "}");
        CompileAndRegScript<tTVPOGLRenderMethod_FillARGB>("FillARGB",
                                                          fillColorShader, 0);
        TEST_SHADER(FillARGB,
                    TVPFillARGB(testdest, 256 * 256, TEST_SHADER_COLOR));

        CompileAndRegScript<tTVPOGLRenderMethod_Script>("FillColor",
                                                        fillColorShader, 0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE, GL_ZERO, GL_ZERO,
                                   GL_ONE);
        TEST_SHADER(FillColor,
                    TVPFillColor(testdest, 256 * 256, TEST_SHADER_COLOR));
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            "FillMask", "void main(){gl_FragColor = vec4(0,0,0,1);}", 0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE,
                                   GL_CONSTANT_COLOR, GL_ZERO);
        TEST_SHADER(FillMask,
                    TVPFillMask(testdest, 256 * 256, TEST_SHADER_OPA));

        // ---------- ConstColorAlphaBlend ----------
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            "ConstColorAlphaBlend", fillColorShader, 0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_CONSTANT_COLOR,
                                   GL_ONE_MINUS_CONSTANT_COLOR, GL_ZERO,
                                   GL_ONE);
        TEST_SHADER(ConstColorAlphaBlend,
                    TVPConstColorAlphaBlend(testdest, 256 * 256,
                                            TEST_SHADER_COLOR,
                                            TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            ...
            ->SetBlendFuncSeparate(
                GL_FUNC_ADD, GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR,
                GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR);
        TEST_SHADER(ConstColorAlphaBlend_a,
                    TVPConstColorAlphaBlend_a(testdest, 256 * 256,
                                              TEST_SHADER_COLOR,
                                              TEST_SHADER_OPA));

        const char *shader_ConstColorAlphaBlend_d =
            "    d.a = opacity + d.a - opacity * d.a;\n"
            "    d.rgb = mix(d.rgb, color.rgb, opacity / (d.a + "
            "0.0001));\n"
            "    gl_FragColor = d;\n"
            "}";
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>(
                ...
                0, common_shader_framebuffer_fetch_prefix);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>(
                "ConstColorAlphaBlend_d",
                opacityPrefix + colorPrefix +
        ...
        TEST_SHADER(
            ...
            texdest->Update(testred, TVPTextureFormat::RGBA, 256 * 4,
                            tTVPRect(0, 0, 256, 256)),
            TVPConstColorAlphaBlend_d(testdest, 256 * 256, TEST_SHADER_COLOR,
                                      TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("CopyColor", copyShader,
                                                        1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE, GL_ZERO, GL_ZERO,
                                   GL_ONE);
        TEST_SHADER(CopyColor, TVPCopyColor(testdest, testdata1, 256 * 256));
        CompileAndRegScript<tTVPOGLRenderMethod_Script>("CopyMask", copyShader,
                                                        1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE, GL_ONE,
                                   GL_ZERO);
        TEST_SHADER(CopyMask, TVPCopyMask(testdest, testdata1, 256 * 256));

        // ---------- ConstAlphaBlend ----------
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            "ConstAlphaBlend", copyShader, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_CONSTANT_COLOR,
                                   GL_ONE_MINUS_CONSTANT_COLOR, GL_ZERO,
                                   GL_ONE);
        TEST_SHADER(ConstAlphaBlend,
                    TVPConstAlphaBlend_HDA(testdest, testdata1, 256 * 256,
                                           TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE, GL_ONE_MINUS_SRC_ALPHA,
                                   GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(ConstAlphaBlend_a,
                    TVPConstAlphaBlend_a(testdest, testdata1, 256 * 256,
                                         TEST_SHADER_OPA));

        CompileAndRegRegularBlendMethod(
            "ConstAlphaBlend_d", opacityPrefix,
            "    d.a = opacity + d.a - opacity * d.a;\n"
            "    d.rgb = mix(d.rgb, s.rgb, opacity / (d.a + "
            "0.0001));\n"
            "    gl_FragColor = d;\n"
            "}");
        TEST_SHADER(ConstAlphaBlend_d,
                    TVPConstAlphaBlend_d(testdest, testdata1, 256 * 256,
                                         TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_AdjustGamma>(
            ...
            "{\n"
            "    vec4 s = texture2D(tex0, v_texCoord0);\n"
            "    s.rgb = clamp(pow(s.rgb, u_gamma) * u_amp + "
            "u_floor, 0.0, "
            "1.0);\n"
            "    gl_FragColor = s;\n"
            "}",
        ...
        TEST_SHADER(AdjustGamma, TVPAdjustGamma(testdest, 256 * 256, &temp),
                    method->SetParameterPtr(
                        method->EnumParameterID("gammaAdjustData"), &data));
        ...
        std::string shader_AdjustGamma_a_2 =
            "    vec3 t = (s.rgb / (s.a + 0.001) - 1.0) * "
            "step(s.rgb, s.aaa + "
            "0.001) + 1.0;\n"
            "    t = clamp(pow(t, u_gamma) * u_amp + u_floor, 0.0, "
            "1.0);\n"
        ...
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_AdjustGamma>(
                "AdjustGamma_a",
                shader_AdjustGamma_a_1 + "    vec4 s = gl_LastFragColor;\n" +
        ...
        // 			method->SetParameterPtr(method->EnumParameterID("gammaAdjustData"),
        // &data)); pass
        ...
        TEST_SHADER_IGNORE_ALPHA(ConstAlphaBlend_SD,
                                 TVPConstAlphaBlend_SD(testdest, testdata1,
                                                       testdata2, 256 * 256,
                                                       TEST_SHADER_OPA));
        RegisterRenderMethod("ConstAlphaBlend_SD_a", method);
        TEST_SHADER(ConstAlphaBlend_SD_a,
                    TVPConstAlphaBlend_SD_a(testdest, testdata1, testdata2,
                                            256 * 256, TEST_SHADER_OPA));
        CompileRenderMethod(
            "ConstAlphaBlend_SD_d",
            (opacityPrefix + ScriptCommonBltPrefix +
             "    s.rgb = mix(s.rgb, d.rgb, d.a * opacity / (s.a * "
             "(1.0 - opacity) * (1.0 - d.a * opacity) + d.a * "
             "opacity + 0.0001));\n" // mix(x,y,a)=x*(1-a)+y*a
        ...
        TEST_SHADER(ConstAlphaBlend_SD_d,
                    TVPConstAlphaBlend_SD_d(testdest, testdata1, testdata2,
                                            256 * 256, TEST_SHADER_OPA));
        ...
        method = CompileAndRegScript<tTVPOGLRenderMethod_UnivTrans>(
            ...
            "    float opa = clamp((rule.r - phase) / vague, 0.0, "
            "1.0);\n"
        ...
        TEST_SHADER_IGNORE_ALPHA(
            ...
            TVPUnivTransBlend(testdest, testdata1, testdata2, testrule,
                              testtable, 256 * 256),
        ...
        TEST_SHADER_IGNORE_ALPHA(
            ...
            TVPUnivTransBlend_switch(testdest, testdata1, testdata2, testrule,
                                     testtable, 256 * 256, 64 + 600, 64),
            method->SetParameterInt(method->EnumParameterID("vague"), 600),
            method->SetParameterInt(method->EnumParameterID("phase"),
                                    64 + 600));
        ...
        method = CompileAndRegScript<tTVPOGLRenderMethod_UnivTrans>(
            ...
            "void main()\n"
            "{\n"
            "    vec4 s1 = texture2D(tex0, "
            "v_texCoord0);\n"
            "    vec4 s2 = texture2D(tex1, "
            "v_texCoord1);\n"
            "    vec4 rule = texture2D(tex2, "
            "v_texCoord2);\n"
            "    float opa = clamp((rule.r - phase) "
            "/ vague, 0.0, 1.0);\n"
            "    s1.rgb = mix(s1.rgb, s2.rgb, s2.a "
            "* (1.0 - opa) / (s2.a * (1.0 - opa) * "
            "(1.0 - s1.a * opa) + s1.a * opa + "
            "0.0001));\n" // mix(x,y,a)=x*(1-a)+y*a
        ...
        TEST_SHADER(
            ...
            TVPUnivTransBlend_d(testdest, testdata1, testdata2, testrule,
                                testtable, 256 * 256),
            method->SetParameterInt(method->EnumParameterID("vague"), 128),
        ...
        TEST_SHADER(
            ...
            (TVPInitUnivTransBlendTable(testtable, 215 + 600, 600),
             TVPUnivTransBlend_switch_d(testdest, testdata1, testdata2,
                                        testrule, testtable, 256 * 256,
                                        215 + 600, 215)),
            method->SetParameterInt(method->EnumParameterID("vague"), 600),
            method->SetParameterInt(method->EnumParameterID("phase"),
                                    215 + 600));
        ...
        TEST_SHADER(
            ...
            (TVPInitUnivTransBlendTable(testtable, 215, 128),
             TVPUnivTransBlend_a(testdest, testdata1, testdata2, testrule,
                                 testtable, 256 * 256)),
            method->SetParameterInt(method->EnumParameterID("vague"), 128),
            method->SetParameterInt(method->EnumParameterID("phase"), 215));
        TEST_SHADER(
...
#define TEST_SHADER_BLTO(x)                                                    \
    TEST_SHADER(                                                               \
        x, TVP##x##_HDA_o(testdest, testdata1, 256 * 256, TEST_SHADER_OPA));
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            "AlphaBlend",
            opacityPrefix + ScriptCommonPrefix +
                "    s.a *= opacity;\n"
        ...
        const char *shader_AlphaBlend_d =
            ...
            "    gl_FragColor = d;\n"
            "}";
        CompileAndRegRegularBlendMethod("AlphaBlend_d", opacityPrefix,
                                        shader_AlphaBlend_d);
        TEST_SHADER(
            AlphaBlend_d,
        ...
        shader_AlphaBlend_d =
            ...
            "    gl_FragColor = d;\n"
            "}";
        CompileAndRegRegularBlendMethod("AlphaBlend_color_d", colorPrefix,
                                        shader_AlphaBlend_d);
        if(GL::glAlphaFunc) {
            CompileAndRegRegularBlendMethod<tTVPOGLRenderMethod_AlphaTest>(
                "AlphaBlend_color_d_AlphaTest", colorPrefix,
                shader_AlphaBlend_d);
        } else {
            CompileAndRegRegularBlendMethod(
                "AlphaBlend_color_d_AlphaTest",
                colorPrefix + alpha_thresholdPrefix,
                "    s *= color;\n"
                "    if(s.a < alpha_threshold) discard;\n"
                "    d.a = s.a + d.a - s.a * d.a;\n"
                "    d.rgb = mix(d.rgb, s.rgb, s.a / (d.a + "
                "0.0001));\n"
                "    gl_FragColor = d;\n"
                "}");
        }
        ...
        // --------- AddBlend ----------
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            "AddBlend", copyShader, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_CONSTANT_COLOR, GL_ONE,
                                   GL_ZERO, GL_ONE);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_REVERSE_SUBTRACT, GL_ONE, GL_ONE,
                                   GL_ZERO, GL_ONE);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE_MINUS_SRC_COLOR,
                                   GL_ZERO, GL_ONE);
        ...
        TEST_SHADER(AdditiveAlphaBlend_a,
                    TVPAdditiveAlphaBlend_ao(testdest, testdata1, 256 * 256,
                                             TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + ScriptCommonBltPrefix +
                ...
                "}",
            2)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR, GL_ZERO,
                                   GL_ONE);
        // TEST_SHADER_BLTO(ColorDodgeBlend); pass it

        CompileAndRegRegularBlendMethod(
            "DarkenBlend", opacityPrefix,
        ...
        CompileAndRegRegularBlendMethod(
            "LightenBlend", opacityPrefix,
            "    d.rgb = mix(d.rgb, max(s.rgb, d.rgb), opacity);\n"
            "    gl_FragColor = d;\n"
            "}");
        TEST_SHADER(LightenBlend,
                    TVPLightenBlend_HDA_o(testdest, testdata1, 256 * 256,
                                          TEST_SHADER_OPA));
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            opacityPrefix + ScriptCommonPrefix +
                ...
                "}",
            1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE, GL_ONE_MINUS_SRC_COLOR,
                                   GL_ZERO, GL_ONE);
        TEST_SHADER_BLTO(ScreenBlend);

        // --------- PsXxxBlend ----------
        const char *shader_PsAddBlend = "    d.rgb = mix(d.rgb, clamp(s.rgb + "
                                        "d.rgb, 0.0, 1.0), s.a * opacity);\n"
                                        "    gl_FragColor = d;\n"
                                        "}";
        CompileAndRegRegularBlendMethod("PsAddBlend", opacityPrefix,
                                        shader_PsAddBlend);
        TEST_SHADER_BLTO(PsAddBlend);

        shader_PsAddBlend = "    s *= color;\n"
                            "    d.rgb = mix(d.rgb, clamp(s.rgb + "
                            "d.rgb, 0.0, 1.0), s.a);\n"
                            "    gl_FragColor = d;\n"
                            "}";
        CompileAndRegRegularBlendMethod("PsAddBlend_color", colorPrefix,
                                        shader_PsAddBlend);
        if(GL::glAlphaFunc) {
            CompileAndRegRegularBlendMethod<tTVPOGLRenderMethod_AlphaTest>(
            ...
            CompileAndRegRegularBlendMethod(
                ...
                "    s *= color;\n"
                "    if(s.a < alpha_threshold) discard;\n"
                "    d.rgb = mix(d.rgb, clamp(s.rgb + d.rgb, 0.0, "
                "1.0), s.a);\n"
                "    gl_FragColor = d;\n"
                "}");
        }
        ...
        const char *shader_PsSubBlend =
            "    d.rgb = mix(d.rgb, clamp(d.rgb + "
            "s.rgb - 1.0, 0.0, 1.0), s.a * opacity);\n"
            "    gl_FragColor = d;\n"
            "}";
        CompileAndRegRegularBlendMethod("PsSubBlend", opacityPrefix,
                                        shader_PsSubBlend);
        TEST_SHADER_BLTO(PsSubBlend);

        shader_PsSubBlend = "    s *= color;\n"
                            "    d.rgb = mix(d.rgb, clamp(d.rgb + s.rgb - 1.0, "
                            "0.0, 1.0), s.a);\n"
                            "    gl_FragColor = d;\n"
                            "}";
        CompileAndRegRegularBlendMethod("PsSubBlend_color", colorPrefix,
                                        shader_PsSubBlend);
        if(GL::glAlphaFunc) {
            CompileAndRegRegularBlendMethod<tTVPOGLRenderMethod_AlphaTest>(
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            "PsMulBlend",
            opacityPrefix + ScriptCommonPrefix + shader_PsMulBlend, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR, GL_ZERO,
                                   GL_ONE);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            "PsMulBlend_color",
            colorPrefix + ScriptCommonPrefix + shader_PsMulBlend, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR, GL_ZERO,
                                   GL_ONE);
        if(GL::glAlphaFunc) {
            CompileAndRegScript<tTVPOGLRenderMethod_AlphaTest>(
                ...
                colorPrefix + ScriptCommonPrefix + shader_PsMulBlend, 1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_SRC_COLOR,
                                       GL_ZERO, GL_ONE);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>(
                "PsMulBlend_color_AlphaTest",
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    vec3 sd = s.rgb * d.rgb;\n"
            "    s.rgb = step(0.5, d.rgb) * ((s.rgb + d.rgb) * 2.0 - "
            "sd * 4.0 "
            "- "
            "1.0) + sd * 2.0;\n"
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    vec3 sd = s.rgb * d.rgb;\n"
            "    s.rgb = step(0.5, s.rgb) * ((s.rgb + d.rgb) * 2.0 - "
            "sd * 4.0 "
            "- "
            "1.0) + sd * 2.0;\n"
            "    d.rgb = mix(d.rgb, s.rgb, s.a * opacity);\n"
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    s.rgb = pow(d.rgb, (1.0 - step(0.5, s.rgb)) * ((1.0 "
            "- s.rgb) "
            "* "
            "2.0 - t) + t);\n"
            "    d.rgb = mix(d.rgb, s.rgb, s.a * opacity);\n"
        ...
        CompileAndRegRegularBlendMethod(
            "PsColorDodgeBlend", opacityPrefix,
            "    s.rgb -= 0.001;\n" // avoid n / 0
        ...
        CompileAndRegRegularBlendMethod(
            "PsColorDodge5Blend", opacityPrefix,
        ...
        CompileAndRegRegularBlendMethod(
            "PsColorBurnBlend", opacityPrefix,
            "    s.rgb += 0.001;\n"
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    d.rgb = mix(d.rgb, max(s.rgb, d.rgb), s.a * "
            "opacity);\n"
            "    gl_FragColor = d;\n"
            "}");
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    d.rgb = mix(d.rgb, min(s.rgb, d.rgb), s.a * "
            "opacity);\n"
            "    gl_FragColor = d;\n"
            "}");
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    d.rgb = mix(d.rgb, abs(s.rgb - d.rgb), s.a * "
            "opacity);\n"
            "    gl_FragColor = d;\n"
            "}");
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    d.rgb += (s.rgb - (s.rgb * d.rgb * 2.0)) * s.a * "
            "opacity;\n"
            "    gl_FragColor = d;\n"
            "}");
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            "PsScreenBlend",
            opacityPrefix + ScriptCommonPrefix + shader_PsScreenBlend, 1)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE_MINUS_DST_COLOR, GL_ONE,
                                   GL_ZERO, GL_ONE);
        ...
        if(GL::glAlphaFunc) {
            CompileAndRegScript<tTVPOGLRenderMethod_AlphaTest>(
                ...
                colorPrefix + ScriptCommonPrefix + shader_PsScreenBlend, 1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ONE_MINUS_DST_COLOR,
        ...
        if(GL_CHECK_shader_framebuffer_fetch) {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>(
                "AdditiveAlphaToAlpha",
                std::string() +
                    "void main() {\n"
                    "    vec4 s = gl_LastFragColor;\n" +
                    shader_AdditiveAlphaToAlpha,
                0, common_shader_framebuffer_fetch_prefix);
        } else {
            CompileAndRegScript<tTVPOGLRenderMethod_Script>(
                "AdditiveAlphaToAlpha",
                ScriptCommonPrefix + shader_AdditiveAlphaToAlpha, 1)
                ->SetTargetAsSrc();
        }
        TEST_SHADER(AdditiveAlphaToAlpha,
                    TVPConvertAdditiveAlphaToAlpha(testdest, 256 * 256));

        CompileAndRegScript<tTVPOGLRenderMethod_Script>("AlphaToAdditiveAlpha",
                                                        "void main(){}", 0)
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_DST_ALPHA, GL_ZERO,
                                   GL_ONE);
        TEST_SHADER(AlphaToAdditiveAlpha,
                    TVPConvertAlphaToAdditiveAlpha(testdest, 256 * 256));

        // DoGrayScale
        const char *shader_DoGrayScale =
            "    float c = s.r * 0.0743 + s.g * 0.7148 + s.b * "
            "0.2109;\n"
            "    s.r = c; s.g = c; s.b = c;\n"
            "    gl_FragColor = s;\n"
            "}";
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_BoxBlur>(
            ...
            "}",
            1) /*->SetTargetAsSrc()*/;
        // TEST_SHADER(BoxBlur, TVPDoGrayScale(testdest, 256 * 256));
        // let it pass
#if 0
        ...
        if(!tempTexture || tempTexture->internalW < w ||
           tempTexture->internalH < h) {
            ...
            tempTexture = new tTVPOGLTexture2D_mutatble(
                nullptr, 0, w, h, TVPTextureFormat::RGBA, 1.f, 1.f);
            // tempTexture = (tTVPOGLTexture2D
            // *)CreateTexture2D(nullptr, 0, w, h,
    ...
    iTVPTexture2D *CreateTexture2D(const void *pixel, int pitch, unsigned int w,
        ...
        if(format > TVPTextureFormat::Compressed) {
            ...
            switch(format) {
                ...
                default:
                    TVPThrowExceptionMessage(
                        TJS_W("Unsupported compressed texture format "
                              "[%1]"),
                        (tjs_int)(format - TVPTextureFormat::Compressed));
                    break;
            }
            if(w <= GetMaxTextureWidth() && h <= GetMaxTextureHeight()) {
                return new tTVPOGLTexture2D_static(
                    pixel, blkw * blkh * blksize,
                    format - TVPTextureFormat::Compressed, w, h, w, h, 1, 1);
            }
            // too large texture, decompress and resize through
            // default route
            ...
            switch(format) {
                ...
                case TVPTextureFormat::Compressed +
                    GL_COMPRESSED_RGBA8_ETC2_EAC:
                    ETCPacker::decodeWithAlpha(pixel, pixeldata, pitch, h, blkw,
                                               blkh);
                    break;
                default:
                    TVPThrowExceptionMessage(
            ...
            iTVPTexture2D *ret = _CreateStaticTexture2D(
                pixeldata, w, h, pitch, TVPTextureFormat::RGBA, opaque);
    ...
    iTVPTexture2D *CreateTexture2D(tTJSBinaryStream *src) override {
        ...
        if(header.width > GetMaxTextureWidth() ||
           header.height > GetMaxTextureHeight()) {
            return nullptr;
        }
        ...
        switch((PVR3TexturePixelFormat)header.pixelFormat) {
            ...
            case PVR3TexturePixelFormat::RGBA4444:
                ...
                fmt = GL_RGBA;
                pixsize = 2;
                pixtype = GL_UNSIGNED_SHORT_4_4_4_4;
                break;
            case PVR3TexturePixelFormat::RGBA5551:
                texfmt = TVPTextureFormat::RGBA;
                fmt = GL_RGBA;
                pixsize = 2;
                pixtype = GL_UNSIGNED_SHORT_5_5_5_1;
                break;
            case PVR3TexturePixelFormat::RGB565:
                texfmt = TVPTextureFormat::RGB;
                fmt = GL_RGB;
                pixsize = 2;
                pixtype = GL_UNSIGNED_SHORT_5_6_5;
                break;
        ...
        if(fmt) {
            ...
            buf.resize(pixsize);
            if(src->Read(&buf.front(), pixsize) != pixsize)
                return nullptr;
            if(fmt == GL_BGRA_EXT &&
               !TVPCheckGLExtension("GL_EXT_texture_format_BGRA8888") &&
               !TVPCheckGLExtension("GL_EXT_bgra")) {
                TVPReverseRGB((tjs_uint32 *)&buf.front(),
                              (tjs_uint32 *)&buf.front(), pixsize / 4);
            }
            tTVPOGLTexture2D_static *ret = new tTVPOGLTexture2D_static(
                texfmt, header.width, header.height, 1, 1);
            ret->InitPixel(&buf.front(), pitch, fmt, pixtype, header.width,
                           header.height);
            return ret;
        }
        ...
        switch((PVR3TexturePixelFormat)header.pixelFormat) {
            ...
            case PVR3TexturePixelFormat::PVRTC4BPP_RGB:
                fmt = GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG;
                eachblkw = 4;
                eachblkh = 4;
                blksize = 8;
                break;
            ...
            case PVR3TexturePixelFormat::PVRTC2_2BPP_RGBA:
                fmt = GL_COMPRESSED_RGBA_PVRTC_2BPPV2_IMG;
                eachblkw = 8;
                eachblkh = 4;
                blksize = 8;
                break;
            case PVR3TexturePixelFormat::PVRTC2_4BPP_RGBA:
                fmt = GL_COMPRESSED_RGBA_PVRTC_4BPPV2_IMG;
                eachblkw = 8;
                eachblkh = 4;
                blksize = 8;
                break;
            ...
            case PVR3TexturePixelFormat::ETC2_RGBA:
                fmt = GL_COMPRESSED_RGBA8_ETC2_EAC;
                eachblkw = 4;
                eachblkh = 4;
                blksize = 16;
                break;
        ...
        if(TVPIsSupportTextureFormat(fmt)) {
            ...
            tTVPOGLTexture2D_static *ret = new tTVPOGLTexture2D_static(
                texfmt, header.width, header.height, 1, 1);
            ret->InitCompressedPixel(&buf.front(), size, fmt, blkw * eachblkw,
                                     blkh * eachblkh);
            return ret;
        }
    ...
    void CopyTexture(tTVPOGLTexture2D *dst, tTVPOGLTexture2D *src,
                     const tTVPRect &rcsrc) {
        if(GL::glCopyImageSubData && !src->IsCompressed &&
           ...
           src->Format == dst->Format) {
            ...
            //             static bool check = false;
            //             if(check) {
            //                 cv::Mat _src(src->internalH, src->internalW,
            //                 CV_8UC4); cv::Mat _dst(dst->internalH,
            //                 dst->internalW, CV_8UC4); GL::glGetTextureImage(
            //                     src->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
            //                     src->internalH * src->internalW * 4,
            //                     _src.ptr(0, 0));
            //                 GL::glGetTextureImage(
            //                     dst->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
            //                     dst->internalH * dst->internalW * 4,
            //                     _dst.ptr(0, 0));
            //                 dst = dst;
            //             }
            // #endif
        ...
        //             GL::glGetTextureImage(src->texture, 0, GL_BGRA,
        //             GL_UNSIGNED_BYTE,
        //                                   src->internalH * src->internalW *
        //                                   4, _src.ptr(0, 0));
        //             GL::glGetTextureImage(dst->texture, 0, GL_BGRA,
        //             GL_UNSIGNED_BYTE,
        //                                   dst->internalH * dst->internalW *
        //                                   4, _dst.ptr(0, 0));
        //             dst = dst;
        //         }
        // #endif
    ...
    bool GetTextureStat(iTVPTexture2D *texture, uint64_t &vmemsize) override {
        ...
        vmemsize =
            texture->GetInternalWidth() * texture->GetInternalHeight() * 4;
        return true;
    }
    ...
    void OperateRect(iTVPRenderMethod *_method, iTVPTexture2D *_tar,
        ...
        // if (!method->CustomProc ||
        // !(method->*(method->CustomProc))(tar, rctar, texlist)) {
        // float tw = (float)tar->internalW, th =
        // (float)tar->internalH;
        ...
        //             for(unsigned int i = 0; i < texlist.size(); ++i) {
        //                 _src[i] = new cv::Mat(texlist[i].tex->internalH,
        //                                       texlist[i].tex->internalW,
        //                                       CV_8UC4);
        //                 GL::glGetTextureImage(
        //                     texlist[i].tex->texture, 0, GL_BGRA,
        //                     GL_UNSIGNED_BYTE, texlist[i].tex->internalH *
        //                     texlist[i].tex->internalW * 4, _src[i]->ptr(0,
        //                     0));
        //             }
        ...
        //             GL::glGetTextureImage(tar->texture, 0, GL_BGRA,
        //             GL_UNSIGNED_BYTE,
        //                                   tar->internalH * tar->internalW *
        //                                   4, _tar.ptr(0, 0));
        //             if(glIsEnabled(GL_STENCIL_TEST)) {
        //                 glReadPixels(0, 0, tar->internalW, tar->internalH,
    ...
    void OperateTriangles(iTVPRenderMethod *_method, int nTriangles,
        ...
        if(method->tar_as_src) {
            ...
            GLVertexInfo &texitem = texlist.back();
            {
                if(reftar) {
                    ...
                    for(int i = 0; i < ptcount; ++i) {
                        pttar.emplace_back(
                            tTVPPointD{ _pttar[i].x - l, _pttar[i].y - t });
                    }
        ...
        for(int i = 0; i < ptcount; ++i) {
            pttar[i * 2 + 0] =
                (((GLfloat)_pttar[i].x - rcl) / rcw - 0.5f) * 2.f;
            pttar[i * 2 + 1] =
        ...
        CHECK_GL_ERROR_DEBUG();
        //		if (tar->GetWidth() == rcclip.get_width() &&
        // tar->GetHeight() == rcclip.get_height()) {
        // 			GLfloat oldClearColor[4] = { 0.0f };
        // 			glGetFloatv(GL_COLOR_CLEAR_VALUE,
        // oldClearColor); 			glClearColor(0.f, 0.f, 0.f,
        // 0.f); 			glClear(GL_COLOR_BUFFER_BIT);
        // glClearColor(oldClearColor[0], oldClearColor[1],
        // oldClearColor[2], oldClearColor[3]); 		} else {
        // static tTVPOGLRenderMethod* _method =
        ...
        // maxx = 1, 				miny = -1,
        // maxy = 1; 			static const GLfloat vertices[8] = {
        // minx, miny, 				maxx, miny,
        // minx, maxy, 				maxx, maxy
        // 			};
        // 			glVertexAttribPointer(method->GetPosAttr(), 2,
    ...
    void OperatePerspective(iTVPRenderMethod *_method, int nQuads,
                            iTVPTexture2D *_tar, iTVPTexture2D *reftar,
        ...
        for(int i = 0; i < nQuads; ++i) {
            ...
            cv::Point2f pts_dst[] = { cv::Point2f(dstpt[0].x * tar->_scaleW,
                                                  dstpt[0].y * tar->_scaleH),
                                      cv::Point2f(dstpt[1].x * tar->_scaleW,
                                                  dstpt[1].y * tar->_scaleH),
                                      cv::Point2f(dstpt[3].x * tar->_scaleW,
                                                  dstpt[3].y * tar->_scaleH),
                                      cv::Point2f(dstpt[2].x * tar->_scaleW,
                                                  dstpt[2].y * tar->_scaleH) };

            cv::Mat perspective_matrix =
                cv::getPerspectiveTransform(pts_dst, pts_src);
            ...
            *(tTVPPointD **)&textures[0].second = &pttex[0];
            ...
            OperateTriangles(method, nQuads * 2, _tar, reftar, rcclip,
                             &pttar[0], textures);
        }
    }
    ...
    void BeginStencil(iTVPTexture2D *reftex) override {
        if(_CurrentFBOValid && _CurrentRenderTarget) {
            ...
            unsigned int w = reftex->GetInternalWidth(),
                         h = reftex->GetInternalHeight();
